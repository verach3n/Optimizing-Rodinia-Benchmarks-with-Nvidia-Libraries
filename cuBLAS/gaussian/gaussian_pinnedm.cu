...
int main(int argc, char *argv[])
{
  printf("WG size of kernel 1 = %d, WG size of kernel 2= %d X %d\n", MAXBLOCKSIZE, BLOCK_SIZE_XY, BLOCK_SIZE_XY);
    ...
    float **AP, **BP, **MP;
    int *Pivot, *Info;
    cudaMallocManaged(&AP, Size * sizeof(float*));
    for (int i = 0; i < Size; i++) {
        cudaMallocManaged(&AP[i], Size * sizeof(float));
    }
    cudaMallocManaged(&BP, Size * sizeof(float*));
    for (int i = 0; i < Size; i++) {
        cudaMallocManaged(&BP[i], Size * sizeof(float));
    }
    cudaMallocManaged<float>(&A, Size*Size*sizeof(float));
    cudaMallocManaged<float>(&B, Size*sizeof(float));
    cudaMallocManaged<int>(&Info, sizeof(int));
    cudaMallocManaged<int>(&Pivot, Size*sizeof(int));
    ...
    status = cublasSgetrfBatched(handle, Size, AP, Size, Pivot, Info, 1);

      //PrintMat(U,Size,Size);
    cublasSgetrsBatched(handle, CUBLAS_OP_N, Size, 1, AP, Size, Pivot, BP, Size, Info, 1);
