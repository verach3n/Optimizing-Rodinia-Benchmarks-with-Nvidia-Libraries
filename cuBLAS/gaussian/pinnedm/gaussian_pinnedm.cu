...
int main(int argc, char *argv[])
{
  printf("WG size of kernel 1 = %d, WG size of kernel 2= %d X %d\n", MAXBLOCKSIZE, BLOCK_SIZE_XY, BLOCK_SIZE_XY);
    ...
    cudaMallocHost((void**)&AP, Size*sizeof(float*));
    for (int i = 0; i < Size; i++) {
        cudaMallocHost(&AP[i], Size * sizeof(float*));
    }
    cudaMallocHost((void**)&BP, Size*sizeof(float*));
    for (int i = 0; i < Size; i++) {
        cudaMallocHost(&BP[i], sizeof(float*));
    }
    cudaMallocHost((void**)&Info, sizeof(int));
    cudaMallocHost((void**)&Pivot, Size*sizeof(int));
